 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N; t++) {
      ...
      //cout<<"Test cost: "<<50 * CppAD::pow(vars[v_start + t] - ref_v, 2)<<endl;
    }
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      ...
      fg[0] += 700*CppAD::pow(vars[delta_start + t] * vars[v_start+t], 2); //adjusting steering acc to velocity
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  size_t n_vars =  N * 6 + (N - 1) * 2; //Asssuming 10 time steps
  ...
  //Assuming 25 degrees for delta
  for (unsigned int i = delta_start ; i < a_start; i++) {
    vars_lowerbound[i] =  -0.436332;
    vars_upperbound[i] =  0.436332;
  }
  // Acceleration/ Deceleration ranging -1 to 1
